...
export default function InboundConfig() {
  ...
  const [formData, setFormData] = useState({ name: '', config_type: 'webhook', config_data: '{}' });

  useEffect(() => { fetchConfigs(); }, []);

  const fetchConfigs = async () => {
    try {
      const response = await api.get('/api/v1/integration/inbound');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) await api.put(`/api/v1/integration/inbound/${editingId}`, data);
      else await api.post('/api/v1/integration/inbound', data);
      fetchConfigs();
      resetForm();
    } catch { alert('保存失败'); }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await api.delete(`/api/v1/integration/inbound/${id}`);
      fetchConfigs();
    } catch { alert('删除失败'); }
  };
  ...
  const resetForm = () => {
    setFormData({ name: '', config_type: 'webhook', config_data: '{}' });
    setEditingId(null);
    setShowForm(false);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-white">入站配置</h2>
      ...
      {showForm && (
        <div className="p-6 rounded-lg bg-dark-card border border-tech-blue/20">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div><label className="block text-sm text-muted-foreground mb-1">类型</label><select value={formData.config_type} onChange={(e) => setFormData({...formData, config_type: e.target.value})} className="w-full px-3 py-2 rounded-lg bg-dark-bg border border-tech-blue/20 text-white"><option value="webhook">Webhook</option><option value="api">API</option><option value="file">文件</option></select></div>
